...
- [ ] §2 的 16 個有入口的綁定，每個都能從新 UI 觸發，並產生表列的可觀察結果。
- [ ] §3 每個畫面元素在新設計都有對應（可以合併位置，不能消失）。
- [ ] §4 每一行都有測試覆蓋且通過；測試改成 `import` 模組後仍斷言同一件事。
- [ ] `node frontend/test/*.mjs` 六個全綠。
- [ ] `wails build` 成功、`go test ./...` 通過。
- [ ] §5 的 1、2、3、5、6 必修；4、8、9、10 視設計；7 的後端部分另開 issue。
